...
function CreatorPage() {
  ...
  const handleGenerateLink = async () => {
    ...
    try {
      ...
      setShareLink(getPublicShareUrl(billId))
    } catch (err) {
      const message =
  ...
  const displayedTotal =
    billTotal ?? items.reduce((sum, item) => sum + item.price, 0)
  const progress =
      ...
      {screen === 'welcome' ? (
        <main className="welcome-page">
          ...
          <section className="welcome-copy">
            ...
            <p>
              No more equal splits when one person ordered a salad and another had
              the steak. Splito keeps every bill item-by-item, fair, and drama-free.
            </p>
      ...
      {screen === 'home' ? (
        <main className="home-page">
          <header className="app-header">
            <button className="icon-button" type="button" onClick={handleBackToWelcome}>
